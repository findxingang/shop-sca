...
@Slf4j
@Service
public class OrderServiceImplV6 implements OrderService {
    @Resource
    ...
    public void saveOrder(OrderParams orderParams) {
        ...
        if (user.getId() == -1){
            throw new RuntimeException("触发了用户微服务的容错逻辑: " + JSONUtil.toJsonStr(orderParams));
        }
        ...
        if (product.getId() == -1){
            throw new RuntimeException("触发了商品微服务的容错逻辑: " + JSONUtil.toJsonStr(orderParams));
        }
